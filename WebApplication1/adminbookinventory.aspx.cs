...
namespace WebApplication1
{
...

    {
        ...
        protected void LinkButton4_Click(object sender, EventArgs e)
        {
            if(TextBox1.Text == "")
            {
                Response.Write("<script>alert('Book ID cannot be empty' );</script>");
            }
            else
            {
                GetBookById();
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Write("<script>alert('Book ID or Name feild cannot be empty' );</script>");
            }
            else if (CheckIFBookExist())
            {
                Response.Write("<script>alert('Book already Exist', Please Chech The Book Exist );</script>");
            }
            else
            {
                AddNewBooks();
        ...
        void FillAuthorPublisherValues()
        {
            try
            {
                ...
                SqlCommand pcmd = new SqlCommand("SELECT publisher_name from publisher_master_tbl;", con);
                SqlDataAdapter adp = new SqlDataAdapter(pcmd);
                DataTable dtbl = new DataTable();
                adp.Fill(dtbl);
                DropDownList2.DataSource = dtbl;
        ...
        void UpdateBookById()
        {
            ...
            else if (checkIfBookExist() == false)
            {
                Response.Write("<script>alert('Invalid Book ID ');</script>");
            }
            else
            {
                try
                {
                    int actual_stock = Convert.ToInt32(TextBox4.Text);
                    int current_stock = Convert.ToInt32(TextBox5.Text);
                    ...
                    else
                    {
                        ...
                        {
                            Response.Write("<script>alert('Actual Stock Stock Value Cannot be less than Issued Books');</script>");
                            return;
                        }
                        else
                        {
